...
public class Util {
  ...
  public static Translation3d flipRedBlue(Translation3d original) {
    return new Translation3d(
        Units.Meter.of(FieldConstants.fieldLength).minus(original.getMeasureX()),
        original.getMeasureY(),
        original.getMeasureZ());
  }
  ...
  public static class MeasureInterpolable<T extends Measure<U>, U extends Unit>
  ...
  public static Pose2d inversePose(Pose2d pose) {
    ...
    return new Pose2d(
        inverseTranslation(pose.getTranslation()).rotateBy(rotation_inverted), rotation_inverted);
  }
  ...
  public static class Pose2dTimeInterpolable {
    ...
    public Time getTimeFromPose(Pose2d pose) {
      ...
      double distanceToPrevPose =
          prevState.getFirst().getTranslation().getDistance(pose.getTranslation());
      double distanceToNextPose =
          nextState.getFirst().getTranslation().getDistance(pose.getTranslation());
      double percentToNextPose =
          (prevState.getFirst().getTranslation().getDistance(nextState.getFirst().getTranslation())
                  - distanceToNextPose)
    ...
    public Pose2d getPoseFromTime(Time time) {
      ...
      double percentIntoDelta =
          time.minus(prevState.getSecond()).in(BaseUnits.TimeUnit)
    ...
    public Pose2dTimeInterpolable(
        Trajectory trajwithTan, Rotation2d startHeading, Rotation2d endHeading) {
      double totalTimeSecpnods = trajwithTan.getTotalTimeSeconds();
      for (State state : trajwithTan.getStates()) {
        Rotation2d poseRotation =
            startHeading.interpolate(endHeading, state.timeSeconds / totalTimeSecpnods);
        poseList.add(
            new Pair<>(
   ...
   * @return An ArrayList containing all intersection points of the circle. ArrayList may have 0, 1,
   *     or 2 values.
   */
  public static ArrayList<Translation2d> getCircleIntersectionPoints(
    ...
    Translation2d solution1 =
        point0.plus(
            new Translation2d(delta.getY(), delta.unaryMinus().getX()).times(h).div(distance));
    Translation2d solution2 =
        point0.plus(
            new Translation2d(delta.unaryMinus().getY(), delta.getX()).times(h).div(distance));

...
import org.team4639._lib.motorcontrol.generic.RSGenericMotorController;

/**
 * Wrapper class for a {@link SparkMax} with automatic SmartDashboard logging and high-level sim
 * support. Make sure to call {@link RSSparkMax#simUpdate()} in {@link
 * Subsystem#simulationPeriodic()}.
 */
public class RSSparkMax implements RSGenericMotorController, Sendable {
  protected int ID;

  protected SparkMax sparkMax;
  ...
  public RSSparkMax(int ID, MotorType motorType) {
    this.ID = ID;
    this.sparkMax = new SparkMax(ID, motorType);
    this.config = new SparkMaxConfig();
    this.resetMode = ResetMode.kResetSafeParameters;
    this.persistMode = PersistMode.kPersistParameters;
    this.name = "RSSparkMax" + this;
  ...
  public RSSparkMax withName(String name) {
    this.name = name;
    return this;
  }
  ...
  public SparkMax getRawSparkMax() {
    return sparkMax;
  }

  public SparkMaxConfig getRawConfig() {
    return config;
  }
   ...
   */
  public double get() {
    if (isSimulation) return sparkMaxSim.getAppliedOutput();
    return sparkMax.get();
  }
  ...
  public REVLibError configure(
    ...
    return sparkMax.configure(config, resetMode, persistMode);
  }
  ...
  public REVLibError configureAsync(
      SparkBaseConfig config, ResetMode resetMode, PersistMode persistMode) {
    return sparkMax.configureAsync(config, resetMode, persistMode);
  }

  /**
   * Returns an object for interfacing with the encoder connected to the encoder pins or front port
   * of the SPARK MAX or the motor interface of the SPARK Max.
   *
  ...
  public SparkClosedLoopController getClosedLoopController() {
    return sparkMax.getClosedLoopController();
  }
  ...
  public REVLibError resumeFollowerMode() {
    return sparkMax.resumeFollowerMode();
  }
  ...
  public REVLibError resumeFollowerModeAsync() {
    return sparkMax.resumeFollowerModeAsync();
  }
  ...
  public REVLibError pauseFollowerMode() {
    return sparkMax.pauseFollowerMode();
  }
  ...
  public REVLibError pauseFollowerModeAsync() {
    return sparkMax.pauseFollowerModeAsync();
  }
  ...
  public boolean hasActiveFault() {
    return sparkMax.hasActiveFault();
  }
  ...
  public boolean hasActiveWarning() {
    return sparkMax.hasActiveWarning();
  }
  ...
  public boolean hasStickyWarning() {
    return sparkMax.hasStickyWarning();
  }
  ...
  public SparkBase.Faults getStickyFaults() {
    return sparkMax.getStickyFaults();
  }
  ...
  public SparkBase.Warnings getStickyWarnings() {
    return sparkMax.getStickyWarnings();
  }
  ...
  public double getBusVoltage() {
    if (isSimulation) return sparkMaxSim.getBusVoltage();
  ...
  public double getOutputCurrent() {
    if (isSimulation) return sparkMaxSim.getMotorCurrent();
  ...
  public REVLibError setCANTimeout(int milliseconds) {
    return sparkMax.setCANTimeout(milliseconds);
  }
  ...
  public int getFirmwareVersion() {
    return sparkMax.getFirmwareVersion();
  }
  ...
  public void setControlFramePeriodMs(int periodMs) {
    sparkMax.setControlFramePeriodMs(periodMs);
  }
  ...
  public String getFirmwareString() {
    return sparkMax.getFirmwareString();
  }
  ...
  public byte[] getSerialNumber() {
    return sparkMax.getSerialNumber();
  }
  ...
  public void setPeriodicFrameTimeout(int timeoutMs) {
    sparkMax.setPeriodicFrameTimeout(timeoutMs);
  }
  ...
  /**
   * Updates the state of the RSSparkMax for simulation purposes. Make sure to call this in the
   * {@link Subsystem#simulationPeriodic()} method.
   */
  public void simUpdate() {
    if (isSimulation) {
      motorSim.setInput(sparkMax.getAppliedOutput() * RoboRioSim.getVInVoltage());
      sparkMaxSim.iterate(motorSim.getAngularVelocityRPM(), RoboRioSim.getVInVoltage(), 0.02);
    }
  }

...
import org.team4639.lib.sim.MechanismSim;
...
public class MotorIOTalonFXSim extends MotorIOTalonFX {
  ...
  private void updateSimStates() {
    main.getSimState()
        .setRotorVelocity(
            sim.mechanismToRotor(sim.getVelocity().times(getMotorInvertMultiplier())));
    main.getSimState()
        .setRawRotorPosition(
            sim.mechanismToRotor(sim.getPosition().times(getMotorInvertMultiplier())));

...
public class MovingLEDPattern implements LEDPattern {
  ...
  public MovingLEDPattern(
      Color color1, Color color2, double frequency, double scale, double brightness) {
    this.color1 = color1;
    this.color2 = color2;
  ...
  public Color get(int led, double time) {
    return Color.lerpRGB(
        Color.kBlack, (waveEquation.apply(led, time) > 0 ? color1 : color2), brightness);
  }
}
